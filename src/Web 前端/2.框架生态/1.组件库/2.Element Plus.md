---
title: Element Plus
createTime: 2025/11/12 21:58:07
permalink: /web/frameworks/cx0bijic/
---
...
<script setup>
// 1. 定义滚动条ref、列表数据
const scrollbarRef = ref(null) // 绑定el-scrollbar的ref

// 2. 核心方法：滚动到底部
const scrollToBottom = async () => {
  if (!scrollbarRef.value) return

  // 确保DOM更新完成（比如新增内容后需要等DOM渲染）
  await nextTick()

  // 获取滚动容器的DOM对象（通过wrapRef）
  const wrapDom = scrollbarRef.value.wrapRef
  if (!wrapDom) return

  // 计算最大滚动距离：内容总高度 - 容器可视高度
  const maxScrollTop = wrapDom.scrollHeight - wrapDom.clientHeight
  // 调用Element Plus内置的setScrollTop方法，设置滚动距离
  scrollbarRef.value.setScrollTop(Math.max(maxScrollTop, 0))
}
</script>
```